...
void wifi_scan(void)
{
    ...
    int rssi_arr[DEFAULT_SCAN_LIST_SIZE] = {0};
    ...
    for (int i = 0; (i < DEFAULT_SCAN_LIST_SIZE) && (i < ap_count); i++)
    {
        ...
        rssi_arr[i] = abs(ap_info[i].rssi); //for sorting
        uint64_t mac = 0;
        mac = ((uint64_t)ap_info[i].bssid[5] << 40) | ((uint64_t)ap_info[i].bssid[4] << 32) | ((uint64_t)ap_info[i].bssid[3] << 24) |
              (ap_info[i].bssid[2] << 16) | (ap_info[i].bssid[1] << 8) | (ap_info[i].bssid[0]);
        ESP_LOGI(TAG, "MAC\t\t0x%" PRIX64 " \n", mac);
